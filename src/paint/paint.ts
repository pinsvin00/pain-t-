...
import { canvas, canvasBuffer, ctx, ctxBuffer, guiLayer } from "./bufferCanvasProvider";
...
export class Paint {
	...
	constructor() {
		...
		this.selectedLayer = new Layer(new Vector2(this.canvas.width, this.canvas.height), new Vector2(0, 0));
	...
	onToolChange() {
		if (this.selectedLayer) {
			...
			while (guiLayer.children.length != 0) {
				guiLayer.removeChild(guiLayer.firstChild);
			}
		}
	}
